...
export const ChatBubble = ({isOwner, message, sender, timestamp}: ChatBubbleProps) => {
    return(
        <div className={` max-w-[75%] px-4 py-0.5 bg-background rounded-lg justify-start items-center gap-2 flex text-justify ${isOwner ? "flex-row-reverse text-end  self-end bg-primary text-background" : "flex-row text-start bg-secondary  self-start text-foreground"} break-all`}>
            <img className="w-10 h-10 rounded-lg border border-gray-300" src="https://via.placeholder.com/40x40" />
            <div className={`self-stretch py-4 flex-col gap-2 flex overflow-hidden ${isOwner ? "items-end" : "items-start"}`}>
